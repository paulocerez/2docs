...
+++ Important: Qdrant and the Vector DB in general has been removed from the project as the focus shifted towards a simpler data storage and retrieval setup +++
...
Auth.js for OAuth authentication. Google as the Identity Provider for Sign-up and login functionality. Neon Serverless as a Database Cloud Service. Firecrawl is used for the crawling procedure. Vercel as the host. In the current implementation models (LLM's) from OpenAI are used.
